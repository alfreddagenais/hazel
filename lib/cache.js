...
module.exports = class Cache {
  ...
  async refreshCache() {
    const { account, repository, pre, token, private_base_url, project_id } = this.config
    const isGitLab = private_base_url.includes('gitlab.com')
    let api_url = null;
    if (isGitLab) {
    ...
    if (token && typeof token === 'string' && token.length > 0) {
      if (isGitLab) {
        headers.Authorization = `PRIVATE-TOKEN: ${token}`
      }else{
        headers.Authorization = `token ${token}`
      }
    }
    ...
    const response = await retry(
      async () => {
        ...
        if (response.status !== 200) {
          throw new Error(
            `GitHub API responded with ${response.status} for url ${api_url}`
          )
        }
